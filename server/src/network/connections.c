...
void *
handle_client (void *client_ctx_ptr)
{
      ...
      if (command)
        {
              ...
              if (filepath)
                {
                  ...
                  if (file)
                    ...
                    {
                      log_all (LOG_ERROR,
                               "Failed to open file: %s (full path: %s)",
                               filepath, full_path);
          ...
          else if (strcmp (command, "PUT") == 0 || strncmp (command, "PUT", 3) == 0)
            {
              ...
              if (dir_slash)
                {
                  *dir_slash = '\0';
                  ...
                  if (stat(dir_path, &st) == 0 && S_ISDIR(st.st_mode))
                    {
                      log_all (LOG_DEBUG, "Directory exists with permissions: %o", st.st_mode & 0777);
                    }
                  else
                    {
                      log_all (LOG_WARNING, "Directory does not exist or is not accessible: %s", dir_path);
                    }
                }
