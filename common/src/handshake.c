...
int nasfs_handshake_unpack_client_hello(const uint8_t *payload,
    ...
    if (!payload || !kex_list_out || !cipher_list_out || payload_len < sizeof(uint16_t) * 2) {
        return -1;
    }

    memcpy(&kex_len_net, payload, sizeof(kex_len_net));
    memcpy(&cipher_len_net, payload + sizeof(kex_len_net), sizeof(cipher_len_net));
...
uint8_t *nasfs_handshake_pack_server_selection(const char *kex_name,
    ...
    total_size = sizeof(uint16_t) * 2 + (size_t)kex_len + (size_t)cipher_len + public_key_len;
...
int nasfs_handshake_unpack_server_selection(const uint8_t *payload,
    ...
    if (!payload || !kex_name_out || !cipher_name_out || !public_key_out || !public_key_len_out ||
