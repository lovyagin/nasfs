...
static int test_client_hello_round_trip(void) {
    ...
    payload = nasfs_handshake_pack_client_hello("Kyber512,ML-KEM-512",
    ...
    if (nasfs_handshake_unpack_client_hello(payload, payload_len, &kex, &cipher) != 0) {
        free(payload);
        return 1;
    }
...
static int test_server_selection_round_trip(void) {
    ...
    payload = nasfs_handshake_pack_server_selection("Kyber512",
                                                    NASFS_CONTROL_CIPHER_XCHACHA20POLY1305,
                                                    public_key,
                                                    sizeof(public_key),
                                                    &payload_len);
    ...
    if (nasfs_handshake_unpack_server_selection(payload,
                                                payload_len,
                                                &kex,
                                                &cipher,
                                                &decoded_key,
