...
int server_socket_setup(server_config_t *config) {
  ...
  if (setsockopt(server_socket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) <
      0) {
    log_all(LOG_ERROR, "setsockopt(SO_REUSEADDR) failed: %s", strerror(errno));
    close(server_socket);
    return -1;
  }
...
void server_run(int server_socket, server_config_t *config) {
  while (server_running) {
    ...
    if (FD_ISSET(server_socket, &readfds)) {
      ...
      if (setsockopt(client_socket, SOL_SOCKET, SO_RCVTIMEO, &client_timeout,
      ...
      if (pthread_create(&client_thread, NULL, handle_client, client_ctx) !=
          0) {
